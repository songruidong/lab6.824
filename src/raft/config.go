...
// shut down a Raft server but save its persistent status.
func (cfg *config) crash1(i int) {
	...
	// pass Make() the last persisted status.
	if cfg.saved[i] != nil {
...
// periodically snapshot raft status
...
// status persister, to isolate previous instance of
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	// new instance's persisted status.
	// but copy old persister's content so that we always
	// pass Make() the last persisted status.
	if cfg.saved[i] != nil {
		cfg.saved[i] = cfg.saved[i].Copy()
...
// as do the threads that read from applyChan.
// returns index.
